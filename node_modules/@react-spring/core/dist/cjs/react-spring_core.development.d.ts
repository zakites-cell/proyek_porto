import { Animatable, Any, Constrain, EasingFunction, ExtrapolateType, Falsy, InterpolatorArgs, InterpolatorConfig, InterpolatorFn, Lookup, Merge, NoInfer, ObjectFromUnion, ObjectType, OneOrMore, Remap, UnknownProps } from "@react-spring/types";
import { FluidProps, FluidValue, Globals, Timeout, createInterpolator, easings, useIsomorphicLayoutEffect, useReducedMotion } from "@react-spring/shared";
import { Animated, AnimatedValue } from "@react-spring/animated";
import * as React$1 from "react";
import { DependencyList, JSX, MutableRefObject, ReactNode, RefObject } from "react";
...
declare class Interpolation<Input = any, Output = any> extends FrameValue<Output> {
  ...
  constructor(/** The source of input values */
...
//#endregion
//#region src/AnimationConfig.d.ts
...
type Springify<T> = Lookup<SpringValue<unknown> | undefined> & { [P in keyof T]: T[P] | SpringValue<T[P]> };
/**
 * The set of `SpringValue` objects returned by a `useSpring` call (or similar).
 */
type SpringValues<T extends Lookup = any> = [T] extends [Any] ? Lookup<SpringValue<unknown> | undefined> : { [P in keyof T]: SpringWrap<T[P]> };
type SpringWrap<T> = [Exclude<T, FluidValue>, Extract<T, readonly any[]>] extends [object | void, never] ? never : SpringValue<Exclude<T, FluidValue | void>> | Extract<T, void>;
//#endregion
...
/** @internal */
type PickEventFns<T> = { [P in Extract<keyof T, EventKey>]?: Extract<T[P], Function> };
//#endregion
...
declare class SpringValue<T = any> extends FrameValue<T> {
  ...
  protected _update({
    ...props
  }: SpringProps<T>, isLoop?: boolean): AsyncResult<SpringValue<T>>;
...
interface ControllerQueue<State extends Lookup = Lookup> extends Array<ControllerUpdate<State, any> & {
  /** The keys affected by this update. When null, all keys are affected. */keys: string[] | null;
}> {}
...
type InvalidKeys<T, U> = { [P in keyof T & keyof U]: T[P] extends U[P] ? never : P }[keyof T & keyof U];
/** Unwrap any `FluidValue` object types */
type RawValues<T extends object> = { [P in keyof T]: T[P] extends FluidValue<infer U> ? U : T[P] };
/**
...
//#endregion
//#region src/TransitionPhase.d.ts
...
//#endregion
//#region src/types/transition.d.ts
...
type GoalValues<T> = FluidProps<T> extends infer Props ? { [P in keyof Props]?: Props[P] | null } : never;
/**
...
interface ControllerProps<State extends Lookup = Lookup, Item = undefined> extends AnimationProps<State> {
   ...
   */
  onStart?: OnStart<SpringValue<State>, Controller<State>, Item> | { [P in keyof State]?: OnStart<SpringValue<State[P]>, Controller<State>, Item> };
  /**
   * Called when the # of animating values hits 0
   *
   * Also accepts an object for per-key events
   */
  onRest?: OnRest<SpringValue<State>, Controller<State>, Item> | { [P in keyof State]?: OnRest<SpringValue<State[P]>, Controller<State>, Item> };
  /**
   * Called once per frame when animations are active
   *
   * Also accepts an object for per-key events
   */
  onChange?: OnChange<SpringValue<State>, Controller<State>, Item> | { [P in keyof State]?: OnChange<SpringValue<State[P]>, Controller<State>, Item> };
  onPause?: OnPause<SpringValue<State>, Controller<State>, Item> | { [P in keyof State]?: OnPause<SpringValue<State[P]>, Controller<State>, Item> };
  onResume?: OnResume<SpringValue<State>, Controller<State>, Item> | { [P in keyof State]?: OnResume<SpringValue<State[P]>, Controller<State>, Item> };
  /**
   * Called after an animation is updated by new props.
...
type PickAnimated<Props extends object, Fwd = true> = unknown & ([Props] extends [Any] ? Lookup : [object] extends [Props] ? Lookup : ObjectFromUnion<Props extends {
  from: infer From;
} ? (From extends (() => any) ? ReturnType<From> : ObjectType<From>) | (TransitionKey & keyof Props extends never ? ToValues<Omit<Props, 'from'>, Fwd> : TransitionValues<Omit<Props, 'from'>>) : TransitionKey & keyof Props extends never ? ToValues<Props, Fwd> : TransitionValues<Props>>);
/**
...
declare const SpringRef: <State extends Lookup = Lookup>() => SpringRef<State>;
//#endregion
//#region src/hooks/useChain.d.ts
/**
...
//#endregion
//#region src/hooks/useSpring.d.ts
/**
...
//#endregion
//#region src/hooks/useSprings.d.ts
...
//#endregion
//#region src/hooks/useSpringRef.d.ts
declare const useSpringRef: <State extends Lookup = Lookup>() => SpringRef<State>;
//#endregion
//#region src/hooks/useSpringValue.d.ts
/**
...
//#endregion
//#region src/hooks/useTrail.d.ts
...
//#endregion
//#region src/hooks/useTransition.d.ts
...
//#endregion
//#region src/hooks/useResize.d.ts
...
//#endregion
//#region src/hooks/useInView.d.ts
...
//#endregion
//#region src/components/Spring.d.ts
...
//#endregion
//#region src/components/Trail.d.ts
...
type Interpolated<T extends ReadonlyArray<any>> = { [P in keyof T]: T[P] extends infer Element ? Element extends FluidValue<infer U> ? U : Element : never };
/**
...
declare const SpringContext: React$1.Context<ISpringContext>;
//#endregion
